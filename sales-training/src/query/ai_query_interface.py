...
class AIQueryInterface:
    ...
    def _handle_date_revenue_query(self, query: str, context: Optional[Dict]) -> Dict:
        ...
        try:            
            ...
            # 3. Lọc dữ liệu theo tháng và năm - HOẠT ĐỘNG VỚI CẢ 2 KIỂU TÊN CỘT
            month_col = 'report_month' if 'report_month' in df.columns else 'month'
            year_col = 'report_year' if 'report_year' in df.columns else 'year'
            
            mask = (df[month_col].astype(float).astype(int) == month_target) & \
                   (df[year_col].astype(float).astype(int) == year_target)
