        ...
        try:
            query = """
                ...
                INNER JOIN scm_sal_data data 
                    ...
                    AND main.masterfn = data.masterfn
        ...
        try:
            query = """
                WITH customer_stats AS (
                    ...
                    GROUP BY masterfn, companyfn, party_unique, party_code, party_desc
                ),
                max_date AS (
            ...
            scope_filters = []
            if effective_companyfn:
                scope_filters.append("companyfn = :companyfn")
                params['companyfn'] = effective_companyfn

            if effective_masterfn:
                scope_filters.append("masterfn = :masterfn")
                params['masterfn'] = effective_masterfn

            if scope_filters:
                query = query.replace(
                    "WHERE tag_void_yn = 'n'",
                    "WHERE tag_void_yn = 'n' AND " + " AND ".join(scope_filters)
                )
