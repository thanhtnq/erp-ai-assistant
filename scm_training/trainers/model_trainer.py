...
class ModelTrainer:
    ...
    def __init__(self, model_dir: str = None):
        ...
        """
        self.model_dir = Path(model_dir or (ARTIFACT_DIR / "_default" / "models"))
    ...
    def train_classifier(
        ...
        try:
            ...
            desired_test_count = max(1, int(round(n_samples * test_size)))
            can_holdout = (
                n_samples >= 5
                and n_classes >= 2
                and desired_test_count >= n_classes
                and n_samples - desired_test_count >= n_classes
            )

            if can_holdout:
                stratify = y if min_class_count >= 2 else None
                X_train, X_test, y_train, y_test = train_test_split(
                    X, y, test_size=desired_test_count, random_state=42, stratify=stratify
                )
            else:
                logger.warning(
                    "Dataset too small for a reliable holdout split; fitting %s on all %s samples",
