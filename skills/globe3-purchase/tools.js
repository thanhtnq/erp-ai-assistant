...
const FILTER_PROPS = {
  ...
  party_desc: { type: 'string', description: 'Vendor name — text search (partial match).' },
...
export default [
  // ── list_purchase_documents ────────────────────────────────────────────────
  {
    ...
    description:
      `List Globe3 ERP purchase documents with filters, sorting, and pagination. ` +
  ...
  {
    name: 'get_purchase_document',
    description:
      'Get a single Globe3 ERP purchase document by uniquenum_pri. ' +
    ...
    async func(args) {
      return ormFetch('findById', 'purchase', {
        masterfn:  args.masterfn,
        companyfn: args.companyfn,
        id:        args.id,
      });
  ...
  // ── count_purchase_documents ───────────────────────────────────────────────
  {
    name: 'count_purchase_documents',
    description:
      `Count Globe3 ERP purchase documents matching filters. Use for "how many" questions. ` +
    ...
    async func(args) {
      return ormFetch('count', 'purchase', {
        masterfn:  args.masterfn,
        companyfn: args.companyfn,
        filters:   args.filters || {},
      });
  ...
  // ── aggregate_purchase_documents ───────────────────────────────────────────
  {
    name: 'aggregate_purchase_documents',
    description:
      `Summarize Globe3 ERP purchase documents — total, average, min, max, or count grouped ` +
      `by vendor, staff, location, currency, date, etc. ` +
      `Specify tag_table_usage: ${TAG_DESCRIPTION}. Use for reports and charts. ` +
      `For "top N vendors/items by value" → always set groupBy=party_code and a limit.`,
    parameters: {
      type: 'object',
      properties: {
        filters: {
    ...
    async func(args) {
      return ormFetch('aggregate', 'purchase', {
        masterfn:  args.masterfn,
        companyfn: args.companyfn,
        filters:   args.filters || {},
        func:      args.func    || 'sum',
